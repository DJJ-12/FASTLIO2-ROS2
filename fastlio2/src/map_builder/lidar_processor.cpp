...
{
    //新增： 第一步：过滤 NaN 点
    CloudType::Ptr cloud_clean(new CloudType);
    std::vector<int> indices;
    pcl::removeNaNFromPointCloud(*package.cloud, *cloud_clean, indices);

    // 第二步：过滤坐标异常点（inf 或超出合理范围的点）
    CloudType::Ptr cloud_valid(new CloudType);
    cloud_valid->reserve(cloud_clean->size());
    for (const auto& pt : cloud_clean->points)
    {
        // 过滤 inf
        if (!std::isfinite(pt.x) || !std::isfinite(pt.y) || !std::isfinite(pt.z))
            continue;
        // 过滤超出合理范围的点（根据你的雷达量程设置，Livox 一般最远200米）
        if (std::abs(pt.x) > 200.0f || std::abs(pt.y) > 200.0f || std::abs(pt.z) > 50.0f)
            continue;
        // 过滤距离雷达太近的点（已有 lidar_min_range 参数，这里双重保险）
        float dist = pt.x * pt.x + pt.y * pt.y + pt.z * pt.z;
        if (dist < 0.01f)  // 距离小于 0.1 米的点
            continue;
        cloud_valid->points.push_back(pt);
    }
    cloud_valid->width = cloud_valid->points.size();
    cloud_valid->height = 1;
    cloud_valid->is_dense = true;
    package.cloud = cloud_valid;
    // m_kf->setLossFunction([&](State &s, SharedState &d)
    //                       { updateLossFunc(s, d); });
    // m_kf->setStopFunction([&](const V21D &delta) -> bool
    //                       { V3D rot_delta = delta.block<3, 1>(0, 0);
