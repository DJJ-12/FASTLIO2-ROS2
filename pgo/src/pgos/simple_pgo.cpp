...
CloudType::Ptr SimplePGO::getSubMap(int idx, int half_range, double resolution)
{
    ...
    // ===== 新增：过滤 NaN 点 =====
    CloudType::Ptr ret_clean(new CloudType);
    std::vector<int> indices;
    pcl::removeNaNFromPointCloud(*ret, *ret_clean, indices);
    ret = ret_clean;
    // ===== 新增：打印包围盒，用于调试溢出原因 =====
    Eigen::Vector4f min_pt, max_pt;
    pcl::getMinMax3D(*ret, min_pt, max_pt);
    std::cout << "[getSubMap] idx=" << idx 
              << " points=" << ret->size()
              << " bbox_min=(" << min_pt[0] << "," << min_pt[1] << "," << min_pt[2] << ")"
              << " bbox_max=(" << max_pt[0] << "," << max_pt[1] << "," << max_pt[2] << ")"
              << " span=(" << (max_pt[0]-min_pt[0]) << "," << (max_pt[1]-min_pt[1]) << "," << (max_pt[2]-min_pt[2]) << ")"
              << std::endl;
